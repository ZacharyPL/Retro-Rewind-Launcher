...
extern "C" {
	...
	extern u8 creditsover_png[];
	extern u8 creditsover2_png[];
...
Menus::Enum MenuMain()
{
	...
	GuiImageData* CreditsTestaImageData;
	GuiImageData* CreditsTestaOverImageData;
	...
	GuiImage* CreditsTestaImage;
	GuiImage* CreditsTestaOverImage;
	...
	CreditsTestaImageData = new GuiImageData(creditsover_png);
	CreditsTestaOverImageData = new GuiImageData(creditsover2_png);
	...
	CreditsTestaImage = new GuiImage(CreditsTestaImageData);
	CreditsTestaOverImage = new GuiImage(CreditsTestaOverImageData);
	...
	OptionsTesta->SetPosition(192, 125);
	...
	UpdateTesta->SetPosition(86, 250);
	...
	LaunchTesta->SetPosition(-21, 125);
	...
	ExitTesta->SetPosition(405, 125);
	ExitTesta->SetTrigger(&Trigger[Triggers::Select]);
	Window->Append(ExitTesta);

	GuiButton* CreditsTesta = new GuiButton(CreditsTestaImageData->GetWidth(), CreditsTestaImageData->GetHeight());
	CreditsTesta->SetImage(CreditsTestaImage);
	CreditsTesta->SetImageOver(CreditsTestaOverImage);
	CreditsTesta->SetAlignment(ALIGN_LEFT, ALIGN_TOP);
	CreditsTesta->SetPosition(299, 250);
	CreditsTesta->SetTrigger(&Trigger[Triggers::Select]);
	Window->Append(CreditsTesta);

	TimeText = new GuiText("", 20, (GXColor){255, 255, 255, 255});
	TimeText->SetAlignment(ALIGN_LEFT, ALIGN_TOP);
	TimeText->SetPosition(18, 420);
	Window->Append(TimeText);
    
    // Display the content of version.txt
    GuiText* versionText = new GuiText("v6.0.17.2", 20, (GXColor){255, 255, 255, 255});
    versionText->SetAlignment(ALIGN_LEFT, ALIGN_TOP);
    versionText->SetPosition(18, 30);
    Window->Append(versionText);
	...
	while(1) {
		...
		else if (CreditsTesta->GetState() == STATE_CLICKED) {
			Window->Remove(OptionsTesta);
			Window->Remove(CreditsTesta);
			Window->Remove(ExitTesta);
			Window->Remove(LaunchTesta);
